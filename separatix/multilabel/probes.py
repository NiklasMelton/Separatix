...
from separatix.multilabel.scoring import (
...
def _linear_classifier(X: Any, config: ProfilerConfig) -> OneVsRestClassifier:
...
def _full_quadratic_classifier(config: ProfilerConfig) -> Pipeline:
    return Pipeline(
        [
            ("poly", PolynomialFeatures(degree=_QUADRATIC_DEGREE, include_bias=False)),
            ("scale", StandardScaler()),
            ("clf", _linear_classifier(np.zeros((1, 1)), config)),
        ]
    )
...
def run_multilabel_model_probes(
    ...
    *,
    ...
    probes: dict[str, Any] = {
        ...
        "linear": _linear_classifier(X_used, config),
        "knn": KNeighborsClassifier(n_neighbors=n_neighbors),
    }
    results: dict[str, dict[str, Any]] = {}
    for name, estimator in probes.items():
        results[name] = _evaluate_probe(
            name,
            estimator,
            X_used,
            Y_used,
    ...
    if dense_info["skipped"]:
        ...
        if (
            ...
            results["smooth_poly"] = _evaluate_probe(
                "PolynomialFeatures+OneVsRestLogisticRegression",
                _full_quadratic_classifier(config),
                dense_X,
                dense_Y,
                config=config,
                budget=budget,
            ...
            if sketch_components is None:
                ...
                results["smooth_poly"] = _evaluate_probe(
                    ...
                    _low_rank_quadratic_classifier(sketch_components, config),
                    dense_X,
                    dense_Y,
                    config=config,
                    budget=budget,
    ...
    if budget["run_kernel_probe"]:
        ...
        if kernel_info["skipped"]:
            ...
            results["kernel_approx"] = _evaluate_probe(
