...
_REGRESSION_HEADLINES = {
    "linear_likely_sufficient": "Linear response model likely sufficient.",
    "smooth_nonlinear_recommended": "Smooth nonlinear response model likely useful.",
    "kernel_or_local_recommended": "Kernel or local regression model likely useful.",
    "high_capacity_or_partitioning_recommended": (
    ...
    "inconclusive": "Regression diagnostic result is inconclusive.",
}
...
def _render_headline(report: DiagnosticReport) -> str:
    """Render a target-aware headline for a canonical recommendation label."""
    if report.class_summary.get("target_type") == "regression":
        return _REGRESSION_HEADLINES.get(
            report.recommendation,
            RECOMMENDATION_LABELS.get(report.recommendation, report.recommendation),
        )
    return RECOMMENDATION_LABELS.get(report.recommendation, report.recommendation)


def _render_suggestions(report: DiagnosticReport) -> list[str]:
    """Return target-aware next-model suggestions for a canonical label."""
    suggestion_map = (
        _REGRESSION_SUGGESTIONS
        if report.class_summary.get("target_type") == "regression"
        else _SUGGESTIONS
    )
    return suggestion_map.get(report.recommendation, suggestion_map["inconclusive"])
...
def _render_multilabel_recommendation(report: DiagnosticReport) -> str:
    ...
    suggestions = _render_suggestions(report)
